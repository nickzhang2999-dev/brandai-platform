...
> **版本**：V0.0.22 | **日期**：2026-09-17 | **状态**：代码与本地阶段验收完成，待合并/发布
>
> **适用分支**：`claude/workspace-first-paint` | **功能基线**：基于最新 `main` 开发
>
...
- **当前成果**：V0.0.22 在 V0.0.21 全量能力之上修复工作台首屏性能：历史版本继续完整保留在开放画布，但显示面使用受权限保护的 WebP 预览、当前图优先和分批加载；原图继续作为编辑与导出的服务端权威来源。
- **开发原则**：品牌约束自动化、服务端权威、异步任务、多租户隔离、契约唯一事实源、真 provider 验收。
- **最大风险**：旧文档、旧分支或旧工作台逻辑可能覆盖 V0.0.18 的最新品牌约束策略；动手前必须确认分支和运行时提交。
- **当前范围**：本文件描述基于最新 `main` 的独立分支 `claude/workspace-first-paint`；合并与正式发布仍是两个独立动作。
...
- 不在工作台增加品牌套件 Logo、颜色或规则的手动选择表单；项目创作素材/参考图的用途面板不属于品牌套件手动配置。
- 不支持输入框 `@品牌套件` 单次约束。
...
| `ProjectAsset`        | projectId、assetId、MEMBER/REFERENCE、usageMode、exactTransform     | 项目素材/参考图关系及调用语义  |
...
1. `ProjectAsset.usageMode` 保存 `EXACT | ADAPTIVE | REFERENCE`，`exactTransform` 保存归一化的位置、宽度、旋转、镜像、裁切和层级；旧关系按 MATERIAL→EXACT、TEMPLATE→REFERENCE 兼容读取。
2. EXACT 的资源字节不进入 AI 服务。Worker 只向模型提供需要预留的构图槽位，模型返回后由 `exact-assets.ts` 使用源像素确定性合成。
3. EXACT 锁定的是对象身份与内容来源，而不是完整画面几何：允许旋转、缩放、镜像、裁切、局部显示和部分位于画框外；不允许生成式重绘、品类替换或静默丢失。
4. ADAPTIVE 与 REFERENCE 都作为真实 multipart 图片进入 `/images/edits`。ADAPTIVE 要求关键主体出现且可识别，但允许融合；REFERENCE 只用于风格、配色和构图，主体可以不出现。
5. 带 EXACT 的生成版本在 `params.baseImageUrl` 保存干净底图，同时记录 `assetUsages`、`appliedExactAssetIds` 与 `exactComposition`。
6. 后续改图以 `baseImageUrl` 为源调用 edit，再重新应用当前 EXACT 布局与既有水印/自动品牌 Logo，避免模型修改锁定素材或产生重复叠层。
7. Web API 与 Worker 双重校验 workspace、生命周期、图片类型和资源可用性；合成层完全在画框外时失败关闭。

#### 6.2.2 工作台尺寸与清晰度

1. 客户端只提交 `sizeSelection`（比例、1K/2K、自定义比例值），不直接提交 provider `quality`。
2. Web API 使用 `packages/contracts/src/generation-size.ts` 解析 12 个预设尺寸或自定义比例，生成服务端权威 `SizeSpec`。
3. 1K 映射 `quality=medium`；2K 宽高各为对应 1K 的 2 倍，并映射 `quality=high`。
4. gpt-image-2 文生图 `/images/generations` 与品牌套件/STRICT 参考图 `/images/edits` 使用相同尺寸和 quality。
5. gpt-image-2 合法自定义尺寸按原尺寸发送，不再使用旧模型的 1024²/1024×1536/1536×1024 吸附。
6. `GenerationVersion.params` 留存 `ratioKey`、`resolutionTier`、`requestedRatio`、requested width/height 和 `quality`；重新生成从这些字段恢复原设置。
...
| `apps/web/src/app/(brandai)/workspace/ResourceUsagePanel.tsx`                | 三种资源用途选择与 EXACT 几何编辑器   |
| `packages/contracts/src/resource-usage.ts`                                   | 资源用途及确定性变换的 Zod 契约       |
| `apps/web/src/lib/exact-assets.ts`                                           | EXACT 源像素确定性合成器              |
| `packages/contracts/src/generation-size.ts`                                   | 12×2 尺寸矩阵、自定义比例和合法性解析 |
...
- 当前分支预览：`https://jasondev-brandai-platform.geole.me/`。
- Web、Worker、AI 是三个独立服务；部署成功必须同时核验三者。
- `/api/health` 应返回 `web=ok`、`ai=ok`，并包含正确 queuePrefix、Worker commit、AI parser revision 和 `generationRevision=gpt-image-2-size-quality-r1`。共享 Docker DNS 下只有同时匹配解析与出图协议的 AI 容器才可被 Worker 使用；找不到兼容容器时失败关闭，禁止回退到可能属于 main 的裸 `ai` 服务并生成错误比例。
...
当前 V0.0.22 代码门禁及既有 V0.0.18 品牌约束验收基线：

| 验证项                 | 结果                             |
| ---------------------- | -------------------------------- |
| L1 contracts + UI      | 254 项通过（248 + 6）            |
| AI pytest              | 155 项通过                       |
| Web typecheck          | 通过                             |
| Web production build   | 通过                             |
| Prisma schema validate | 通过                             |
| 真浏览器核心画布交互   | 30/30 通过                       |
| 33 图预览压力测试      | 通过（mock 交互数据，仅验传输） |
...
4. 确认目标是 `jasonDEV_0.0.21` 还是另建分支；不得自动合并 main，也不得改写 `jasonDEV_0.0.20`。
...
> 你接手 BrandAI V0.0.22，代码基线在 `claude/workspace-first-paint`，基于最新 `main`，尚未合并。先完整阅读 `docs/产品功能.md`、`docs/产品文档.md`、`CLAUDE.md` 和 `README.md`。产品主流程为：品牌套件 → 项目 → 素材/模板 → AI 工作台 → 自动品牌约束生图 → 画布改图 → 合规/审阅 → 终选导出。品牌套件由系统自动调用；项目创作资源由用户逐项选择 EXACT/ADAPTIVE/REFERENCE。EXACT 锁定身份与源像素，但允许确定性旋转、缩放、镜像、裁切和局部显示，禁止 AI 重绘或替换。画布显示使用受鉴权的紧凑预览，主 Logo、EXACT、改图和导出仍使用原始素材。所有慢调用进入 BullMQ Worker，所有资源访问做 workspace 成员校验，验收使用真 provider。请先核对当前分支和现状，再执行新的开发任务。
