...
> **版本**：V0.0.21 | **日期**：2026-07-28 | **状态**：已落地并通过阶段验收
>
> **适用分支**：`jasonDEV_0.0.21` | **功能基线**：随当前分支 HEAD 更新
>
...
- **当前成果**：V0.0.21 在 V0.0.20 尺寸/清晰度链路之上，将项目素材与参考图收敛为 EXACT、ADAPTIVE、REFERENCE 三种用户可见且真实执行的语义；EXACT 通过干净底图和确定性重合成锁定对象身份，仍允许旋转、缩放、镜像、裁切和局部显示。
- **开发原则**：品牌约束自动化、服务端权威、异步任务、多租户隔离、契约唯一事实源、真 provider 验收。
- **最大风险**：旧文档、旧分支或旧工作台逻辑可能覆盖 V0.0.18 的最新品牌约束策略；动手前必须确认分支和运行时提交。
- **当前范围**：本文件描述独立分支 `jasonDEV_0.0.21`，不得据此认定 `main` 或 `jasonDEV_0.0.20` 已包含这些能力。
...
当前 V0.0.21 代码门禁及既有 V0.0.18 品牌约束验收基线：

| 验证项                 | 结果                             |
| ---------------------- | -------------------------------- |
| L1 contracts + UI      | 212 项通过（206 + 6）            |
| AI pytest              | 135 项通过                       |
| Web typecheck          | 通过                             |
...
> 你接手 BrandAI V0.0.21，代码基线在 `jasonDEV_0.0.21`，尚未合并 main，`jasonDEV_0.0.20` 必须保持不变。先完整阅读 `docs/产品功能.md`、`docs/产品文档.md`、`CLAUDE.md` 和 `README.md`。产品主流程为：品牌套件 → 项目 → 素材/模板 → AI 工作台 → 自动品牌约束生图 → 画布改图 → 合规/审阅 → 终选导出。品牌套件由系统自动调用；项目创作资源由用户逐项选择 EXACT/ADAPTIVE/REFERENCE。EXACT 锁定身份与源像素，但允许确定性旋转、缩放、镜像、裁切和局部显示，禁止 AI 重绘或替换。主 Logo 与 EXACT 都在模型返回后使用原始素材确定性合成。所有慢调用进入 BullMQ Worker，所有资源访问做 workspace 成员校验，验收使用真 provider。请先核对当前分支和现状，再执行新的开发任务。
