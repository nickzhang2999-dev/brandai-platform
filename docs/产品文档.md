...
> **版本**：V0.0.20 | **日期**：2026-07-28 | **状态**：已落地并通过阶段验收
>
> **适用分支**：`jasonDEV_0.0.20` | **功能基线**：随当前分支 HEAD 更新
>
...
- **当前成果**：V0.0.20 在 V0.0.18 品牌约束闭环之上补齐工作台 12 个预设比例、自定义比例、1K/2K 与真实 quality 映射；品牌规则到模型、Logo 合成和合规检查链路保持不变。
- **开发原则**：品牌约束自动化、服务端权威、异步任务、多租户隔离、契约唯一事实源、真 provider 验收。
- **最大风险**：旧文档、旧分支或旧工作台逻辑可能覆盖 V0.0.18 的最新品牌约束策略；动手前必须确认分支和运行时提交。
- **当前范围**：本文件描述独立分支 `jasonDEV_0.0.20`，不得据此认定 `main` 已包含这些能力。
...
当前 V0.0.20 代码门禁及既有 V0.0.18 品牌约束验收基线：

| 验证项                 | 结果                             |
| ---------------------- | -------------------------------- |
| L1 contracts + UI      | 207 项通过                       |
| AI pytest              | 134 项通过                       |
| Web typecheck          | 通过                             |
...
4. 确认目标是 `jasonDEV_0.0.20` 还是另建分支；不得自动合并 main。
...
> 你接手 BrandAI V0.0.20，代码基线在 `jasonDEV_0.0.20`，尚未合并 main。先完整阅读 `docs/产品功能.md`、`docs/产品文档.md`、`CLAUDE.md` 和 `README.md`。产品主流程为：品牌套件 → 项目 → 素材/模板 → AI 工作台 → 自动品牌约束生图 → 画布改图 → 合规/审阅 → 终选导出。品牌套件在工作台中由系统自动调用：有已确认规则走 `BRANDED/branded_direct`，无规则走 `FREE/direct`；禁止新增 Logo/颜色/规则选择按钮。主 Logo 先进入模型视觉上下文，再由 Worker 使用原始素材确定性合成。所有慢调用进入 BullMQ Worker，所有资源访问做 workspace 成员校验，契约同步 Zod/Pydantic，验收使用真 provider。请先核对当前分支和现状，再执行新的开发任务。
