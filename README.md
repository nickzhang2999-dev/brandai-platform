...
**V0.0.17 品牌套件交互收敛（2026-07-18）**：品牌套件页面改为 Lovart 式双状态：空套件只展示六类入口与整本手册上传；已有内容时空白引导完全消失，改为品牌标题、紧凑手册导入条、设计/品牌指南与 Logo、颜色、字体、图像素材架。左侧“我的品牌套件”使用卡片列表，支持新建、改名、删除和切换；图片在当前页面灯箱中放大、缩小和下载。移除页面内手动文字共创、快捷提示、AI 知识摘要与品牌预览入口，PDF 解析、规则确认及项目级品牌约束逻辑保持不变。

**V0.0.17 CDS 数据库启动安全修复（2026-07-18）**：灰度环境的历史 Web profile 仍调用根命令 `pnpm db:push`。该兼容入口现安全转发到 `prisma migrate deploy`，避免 Prisma 因检测到遗留列而阻止 Web 启动，也禁止通过 `--accept-data-loss` 删除现有业务数据；本地一次性数据库仍可直接调用 `pnpm --filter @brandai/db push`。
...
| D17 | Lovart 式品牌套件双状态与套件管理 | 用户 2026-07-18 | ✅ | `brand-knowledge/page.tsx` + `brand-sidebar.tsx` + `brand-context.tsx` | 空套件只显示开始引导；完成态只显示紧凑导入条和六类内容；左侧套件卡支持新建/改名/删除/切换；Logo/图像原位灯箱预览、缩放与下载；不改 PDF 解析及项目约束逻辑 | V0.0.17 2026-07-18 |
