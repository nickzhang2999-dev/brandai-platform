...
> **当前功能版本：V0.0.13**（2026-07-17）
>
> V0.0.13：迁移 prd_agent「视觉创作」右侧对话能力 → 工作台「AI 设计师」对话面板（图生图 / 多图生图 ≤8 张有序引用，单/多图共用同一条 `/images/edits` multipart 路径与解析器，**刻意不搬运** prd_agent 多图 Vision 分支 bug 与「引用块/文件名泄漏进可见消息」bug——展示文本与模型 prompt 物理分离）+ 管理员可配置「图像系统提示词」（`AppSetting.imageSystemPrompt` → 每次出图注入 prompt 首部）。TDD：pytest `test_multi_image_input.py`/`test_system_prompt.py` + 契约 `chat-generation.test.ts` 先红后绿。计划与验收标准见 `docs/12_视觉创作对话面板迁移计划_V0.0.13.md`。
...
> V0.0.9 起模板库参考图走 `templateReferenceAssetIds` 作为 AI inspiration；素材库文件走 `watermarkOverlays` 在 worker 端确定性合成，不再依赖模型复现素材。V0.0.11 补齐二次编辑链路的水印叠加。V0.0.12 进一步把“只参考 / 绝对调用 / 适配调用”拆清：只参考走模板库入口，绝对/适配调用走素材水印叠加；绝对调用不允许 AI 改内容，但显示尺寸和位置可由用户手动调整。
...
| A2 | 用户信息区（头像/姓名/职位/个人菜单） | P01-M02 | ✅ | `brand-sidebar.tsx`（注入 `user.name`）+ `/account` | 姓名已显示；个人菜单：**账号设置**（`/account`，全员）+ **管理后台**（`/admin`，仅 admin 可见，`isAdmin` 由布局注入）+ 退出。侧栏齿轮由 `/admin/settings` 改指 `/account`，个人 vs 管理入口分离。V0.0.12 支持在账号设置修改昵称，默认昵称仍来自邮箱前缀，保存后首页/侧栏读取 DB `User.name` | 接入 2026-06-21；菜单分离 2026-07-07；昵称编辑 2026-07-12 |
...
|---|---|---|---|---|---|---|
| B1 | 问候语区 | P01-M04 | ✅ | `app/(brandai)/page.tsx:33` | 「你好，{user.name}」读取服务端 DB 用户昵称；用户未改昵称时沿用邮箱前缀，修改账号昵称后刷新首页即时生效 | V0.0.12 昵称编辑 2026-07-12 |
...
| F12 | 修改优化（改图） | 一期闭环 | ✅ **已验收** | `page.tsx` → versions/[id]/edit（OpenAI multipart） | 子版本 parentVersionId。V0.0.12 明确支持基于当前/历史出图继续整图修改，用户选中上一版图后进入 `IMAGE_EDIT` 改图链路，而不是每次从文字生图重新开始 | V0.0.12 图生图 2026-07-12 |
...
| F19 | AI 设计师对话面板（迁移自 prd_agent 视觉创作右侧对话，对标 Lovart） | V0.0.13 迁移 | ✅ | `workspace/ChatPanel.tsx` + `lib/chat-composer.ts` + `workspace/page.tsx` 右栏双 Tab + `Generation.chatContext`（Prisma 迁移 `20260717000000`） | 会话流 = Generation 历史投影（不建消息表，服务端权威，刷新/深链可恢复）；**V0.0.13b 富文本混排（用户验收反馈）**：contentEditable 输入器，图片 chip 在光标处插进句子里（「把[chip]放在[chip]的背景里」），线格式用 U+FFFC 标记 chip 位置、气泡按位置行内还原；模型层 brief 标记替换为 `[图N]` 与 multipart 顺序对应（**规避 prd_agent「引用块/文件名泄漏进可见消息」bug**：展示/模型两层物理分离，契约+纯函数测试锁死）；失败气泡带可读原因 + 重试；生成中显示已等待秒数；输入区预留底距避开队列 widget 遮挡。本地全栈 Chromium 真测 16 断言全过。**V0.0.13c 交互物理移植（四轮验收反馈）**：① 对话流只投影 `chatContext` 来源的 Generation（表单历史 brief 不再混入对话流）；② 两阶段选择从 prd_agent TwoPhaseRichComposer 逐行移植——点画布/变体条/历史条图片→灰待选 chip（60% 透明）→点输入区/再点同图/发送→实体 violet 确认→第三次点取消，待确认徽标+清除，源图灰罩✓/序号角标双向同步；③ 底栏物理移植：尺寸 chip（1K·1:1/2:3/3:2 popover→targets 真实尺寸）+ 模型 chip + 圆形发送。本地 Chromium 自验收 27 断言全过（含尺寸 2:3 落库 1024×1536、「切 Tab 不凭空插 chip」回归——灰度自验收捕获的选择同步重放 bug，已拆 onUserPickVersion 真实点击通道修复）**V0.0.13d 删生成面板 + 交互一比一 + 画布持久化（五轮验收）**：AI 设计师成唯一默认右栏（表单/草稿/undo-redo 面板体系移除）；两阶段改为 prd_agent 真语义（replace/Shift 累加/点输入区确认/点空白清待选，Workflow 3 子智能体逐行提取规格后实施）；画布图片本体 hover 边框/灰罩✓/序号角标；上传四途径 + 上传即自动预选；`ProjectCanvas` 服务端持久化（1200ms 防抖，手动元素/相机/已删版本集刷新恢复，上传图带 assetId）。本地 27 断言自验收全过。**V0.0.13e（六轮）**：画布下方 变体条/历史条/终选面板 全删（工作台=画布+对话，与视觉创作一致）；出图 tile 跨 generation 累积在画布（随画布持久化）；终稿/导出/审阅挪进画布选中浮动工具条（BrandAI 特有业务的唯一兼容点，已申报）。31 断言全过。**V0.0.13f（七轮）**：chip 改为可序列化文本 token——复制/剪切得 `[@image:#N:KIND_id:url]` 混合文本，粘贴还原实体 chips（外部 token 保持纯文本防幻觉引用）；同图可多次引用（确认后再点=新 chip，imageInputs 允许重复 id）。12 断言 token e2e 全过 | 新增 2026-07-17；富文本混排 2026-07-17；交互物理移植 2026-07-18；删面板+持久化+画布累积 2026-07-18；token 复制粘贴+同图多引用 2026-07-18 |
| F20 | 图生图 / 多图生图（≤8 张有序引用，单/多图同路径） | V0.0.13 迁移 | ✅ | `CreateGenerationInput.imageInputs` + `generations/route.ts`（IDOR 校验）+ `generate.worker.ts`（折 STRICT refs）+ `apps/ai/main.py`（放开单 STRICT 限制，全量 refs → `generate_with_references`）| 引用历史出图版本/素材 → worker 折成 `mode=STRICT, note=IMAGE_INPUT:{序号}` → `/images/edits` 多 `image[]` multipart。**不搬运 prd_agent 多图 bug**：无 chat/completions Vision 分支，单/多图同一解析器；IMAGE_INPUT 用「按指令合成」模板，遗留素材 STRICT 保留「锁定资产」模板互不污染。pytest `test_multi_image_input.py` 6 用例 | 新增 2026-07-17 |
| F21 | 图像系统提示词（admin 可配置，全局注入出图 prompt） | V0.0.13 迁移 | ✅ | `AppSetting.imageSystemPrompt` + `/admin/settings/ai` 表单 + `lib/settings.ts` + worker → `GenerateRequest.systemPrompt` → `main.py` prompt 首部 | DB > `IMAGE_SYSTEM_PROMPT` env > 不注入；`GenerationVersion.params.prompt` 可查证（A7）。pytest `test_system_prompt.py` 3 用例 | 新增 2026-07-17 |
