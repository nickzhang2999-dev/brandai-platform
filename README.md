...
> **当前功能版本：V0.0.17**（2026-07-18）

以 **项目** 为中心，围绕 **品牌套件 / 素材库 / 模板库 / AI 工作台 / 生成图** 组织品牌广告物料的生成、修改与归档。品牌套件由 **logo / 字体 / 颜色 / 设计指南 / 图像 / 品牌指南** 6 个维度组成。V0.0.17 将品牌手册导入升级为完整 PDF 图文解析：逐页读取、自动裁切视觉资产、生成六类规则草稿，并在确认后作为项目级品牌约束进入出图。

**V0.0.17（2026-07-18）**：对齐 Lovart 品牌套件的主导入路径，默认展示 3×2 六类内容入口与完整品牌手册上传区，手动规则降为次级入口；PDF 由异步 worker 逐页提取文字并渲染页面，VLM 分批识别后合并为 logo / 字体 / 颜色 / 设计指南 / 图像 / 品牌指南六类草稿，同时将模型选中的 Logo、色卡、字体样张、版式和摄影区域裁切为隔离的 `BRAND_KIT` 资产；用户确认规则后，规则文本、主 Logo 和图像参考自动进入当前品牌下所有项目的生成约束。部署环境已用 101 页《一家一居VI全 预览版》完成真实浏览器验收：准确提取 8mm 最小尺寸、6 个字体名称、`#FF6C2C` / `#A1D0CA` / `#3B3C44` 三个标准色、版式/小白砖图像特征与两条品牌文案；六类规则全部确认后，工作台显示项目级自动应用 6 条规则，并由 GPT Image 2 成功生成 4 个真实海报变体。

**V0.0.17 品牌套件交互收敛（2026-07-19）**：品牌套件页面改为 Lovart 式双状态：空套件只展示六类入口与整本手册上传；已有内容时空白引导完全消失，改为可就地改名的品牌标题、紧凑手册导入条、设计/品牌指南与 Logo、颜色、字体、图像素材架。以浏览器同视口实测尺寸为准，完成态有效内容宽度约 640px，素材与“+”入口统一为 120px 方卡，去除标题统计、分类数量、状态圆点和完成态指南新增按钮。左侧改为 60px 全局图标栏 + 260px“我的品牌套件”卡片栏，支持新建、改名、删除和切换；图片在当前页面灯箱中放大、缩小和下载。页面中彻底移除手动文字共创、快捷提示、AI 知识摘要与品牌预览入口，PDF 解析、规则确认及项目级品牌约束逻辑保持不变。
...
**V0.0.15（2026-07-17）**：AI 工作台右侧取消制作模式、需求卖点、场景、生成数量、多渠道尺寸等表单项，改为上方历史过程、下方输入框的干净对话区；隐藏 CDS 技术预览条等用户不需要看到的后台状态；品牌套件内容按 logo、字体、颜色、图像等维度优先展示图片/色块/字体预览；取消品牌套件 PDF 20MB 前端限制，上传 PDF 后直接进入解析流程。
...
进度表 137 行：**✅ 全部完成，🟡 = 0，❌ = 0。**
- **Phase-1 产品方案功能**：全部完成 + 灰度真验（出图 gpt-image-2 / 改图 / 终选 / 导出、通知 A3、推荐品牌 L2、素材文件夹 E3、素材 AI 标注 E9/E10、brief 拆解 B2、campaign 摘要 C8、弹窗体系 H4/H7/H9/H12、模板库 G1 …；品牌预览 D10 已按用户要求退出页面）。
...
| 编号 | 功能点 | 来源 | 状态 | 路径 | 备注 · 验收 | 变更 |
|---|---|---|---|---|---|---|
| D1 | ~~AI 共创输入区~~ | P03-M02 | 🗑 | 后端规则接口保留 | 与 Lovart 空白态/完成态冲突，品牌套件页面不再展示手动文字共创 | 作废 2026-07-18 · 用户明确删除旧入口 |
| D2 | ~~快捷提示词~~ | P03-M03 | 🗑 | — | 随手动文字共创入口一并退出品牌套件页面 | 作废 2026-07-18 |
...
| D10 | ~~品牌预览（综合视觉自动生成）~~ | P03-M11 | 🗑 | 后端生成链路暂保留，页面入口移除 | 用户明确品牌套件不需要独立“品牌预览”模块 | 作废 2026-07-18 |
| D11 | ~~AI 知识摘要~~ | P03-M12 | 🗑 | — | 完成态直接展示六类真实内容，不再增加重复摘要区 | 作废 2026-07-18 |
...
| D17 | Lovart 式品牌套件双状态与套件管理 | 用户 2026-07-18/19 | ✅ | `brand-knowledge/page.tsx` + `brand-sidebar.tsx` + `brand-context.tsx` | 空套件只显示开始引导；完成态为约 640px 编辑区和 120px 素材卡；左侧 60px 全局图标栏 + 260px 套件卡支持新建/改名/删除/切换；标题可就地改名，Logo/图像原位灯箱预览、缩放与下载；页面不再出现品牌预览；不改 PDF 解析及项目约束逻辑 | V0.0.17 同视口校准 2026-07-19 |
...
| L4 | ~~8 类通用富卡~~ → Lovart 六类紧凑素材架 | 设计已收敛 | D4-D9 / D17 | ✅ 六类素材架；D10 品牌预览已按用户要求退出页面 |
