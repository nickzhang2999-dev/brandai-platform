...
| F19 | AI 设计师对话面板（迁移自 prd_agent 视觉创作右侧对话，对标 Lovart） | V0.0.13 迁移 | ✅ | `workspace/ChatPanel.tsx` + `workspace/page.tsx` 右栏双 Tab + `Generation.chatContext`（Prisma 迁移 `20260717000000`） | 会话流 = Generation 历史投影（不建消息表，服务端权威，刷新/深链可恢复）；气泡只显示用户原文 + 引用图 chip（**规避 prd_agent「引用块/文件名泄漏进可见消息」bug**：`chatDisplayText` 与模型 prompt 物理分离，契约测试锁死）；失败气泡带可读原因 + 重试；生成中显示已等待秒数。灰度真验待跑（docs/12 A9） | 新增 2026-07-17 |
