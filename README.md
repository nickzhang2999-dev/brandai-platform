...
|---|---|---|---|---|
| 一期业务闭环（§J） | 6/6 | — | — | — |
| 平台 / 后端能力（§I） | 绝大多数 | ingest 等少量 | — | 计费UI |
| 五页产品模块（§B–F，按设计稿细粒度） | 核心功能可用 | 富展示/联动多处退化 | 推荐品牌/通知/模板库/风格词/参考素材等 | — |
| 通用交互（§H 弹窗/AI输入/卡片） | 部分 | 多数 | 多数弹窗 | — |

> 一句话结论：**业务主干（创建→沉淀→素材→出图→改图→终选→交付）已真实跑通且安全收尾**；**设计稿里的"富展示 / 跨模块联动 / 弹窗体系"还大量未做**（详见 §L 反推清单）。
...
| 编号 | 功能点 | 来源 | 状态 | 路径 | 备注 · 验收 | 变更 |
|---|---|---|---|---|---|---|
| F1 | 顶部项目路径（breadcrumb） | P05-M02 | 🟡 | `workspace/page.tsx`（`?project=` 参数） | 接收 project，breadcrumb 富展示弱 | 2026-06-20 |
| F2 | 顶部基础操作（撤销/重做/缩放/通知） | P05-M03 | ❌ | — | 未做 | 2026-06-20 |
