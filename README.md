...
| L8 | ~~素材↔工作台/Campaign 无联动~~ → 素材水印 / 模板参考 / 生成图三分法已接 | 设计有·**已补** | E11/E12/E14/F9/H7/G1/A1 | ✅ V0.0.9 明确三类图片：素材库 `MATERIAL` 只做水印叠加；模板库 `TEMPLATE` 只做 AI inspiration；生成图 `GENERATED` 进入独立「生成图」入口，不混入素材库默认列表；V0.10 补齐生成图检索和项目维度过滤 |
