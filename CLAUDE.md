...
- **协作(G6)**：`(app)` owner-only 守卫放行被邀成员 / `getOrCreateActiveBrand` 先解析 `Membership` 再兜底建品牌 / 项目导出(交付归档)只放行 final/approved 版本(职责分离,防协作者导出未审批草稿)。
...
- **多尺寸**：记录 OpenAI snap 后的真实尺寸 / `regenerate` 保留原 `targets`+`textMode` / `targets` 数量与尺寸加上界(防直连无界批量烧 provider，绕过 versionCount≤8)。
