...
- **PDF 知识库(parse-manual)**：支持 note-only 证据需成套改两处——① `_coerce_recognize` 现在会丢弃无 `assetId` 的证据;② `Evidence.assetId` 改 optional(Zod+Pydantic 两侧),否则真 VLM 的 note-only 引用要么被丢、要么在 web worker 回填前 400。接 PDF 知识库时一并做。
