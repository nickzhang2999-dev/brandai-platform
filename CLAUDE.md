...
- **协作(G6)**：`(app)` owner-only 守卫放行被邀成员 / `getOrCreateActiveBrand` 先解析 `Membership` 再兜底建品牌。
- **健壮性**：首个 admin bootstrap 原子化(并发竞态) / Campaign Kit `runPrecheck` 移出请求路径(违反 §2,需 worker 异步化)。
- **多尺寸**：记录 OpenAI snap 后的真实尺寸 / `regenerate` 保留原 `targets`+`textMode`。
