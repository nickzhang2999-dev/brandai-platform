...
5. **提需求调整须自带归因代号** —— 凡建议改/加/补需求，标一个代号 + 一句根因，记入 `docs/10_需求调整归因表.md`：**N** 新增需求 · **S** 需求没想清楚 · **I** 没按需求做 · **O** 可见性·验收缺口（做了但页面上看不到/没法确认） · **R** 优化·重构提议。归因不裁决，聚合后看分布定基础设施投向（O/S 多→CDS 预览+视觉测试+DoD 前置；I 多→CI 回归）。
